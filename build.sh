...
grep -rl 'string.atoi' tools/dct --include='*.py' | xargs -r sed -i 's/string\.atoi(\([^)]*\))/int(\1)/g' || true
sed -i 's|\$(python) |true |' scripts/drvgen/drvgen.mk && grep -c 'true ' scripts/drvgen/drvgen.mk
cd ..
